    ...
    handleSubmit = () => {
      if (this.state.username && this.state.password) {
        ...
        fetch(`${process.env.REACT_APP_API_IP}:${process.env.REACT_APP_API_PORT}/login`, { method: 'POST', body: body, headers: {'Content-Type': 'application/json'}})
            ...
            .then(res => {console.log("hello", res);localStorage.setItem('Jwt', res.token); localStorage.setItem('userId', res.userId); localStorage.setItem('userType', res.userType); localStorage.setItem('pseudo', this.state.username);localStorage.setItem('theme', res.theme); this.handleResponse(res)})
